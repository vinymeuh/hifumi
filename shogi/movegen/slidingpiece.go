...
var (
	...
	BlackRookHMoveRules = SlidingPieceMoveRules{
		MagicsTable: NewMagicsTable(RookHMagics, RookHAttacksMask, RookHAttacksWithBlockers),
		PromoteFunc: func(from, to shogi.Square) (can, must bool) {
			switch {
			case from.Rank() <= 3 || to.Rank() <= 3:
				can = true
	...
	BlackRookVMoveRules = SlidingPieceMoveRules{
		MagicsTable: NewMagicsTable(RookVMagics, RookVAttacksMask, RookVAttacksWithBlockers),
		PromoteFunc: func(from, to shogi.Square) (can, must bool) {
			switch {
			case from.Rank() <= 3 || to.Rank() <= 3:
				can = true
